...
with t1:
    ...
    # 4. THE INDEPENDENT PLAYER (STRICT PLACEMENT)
    # This 'st._bottom' block ensures it sits under the chat_input bar
    with st._bottom:
        last_assistant_msg = next((m["content"] for m in reversed(st.session_state.messages) if m["role"] == "assistant"), None)
        
        if last_assistant_msg:
            # Only trigger ElevenLabs if the message has changed
            if st.session_state.get("active_audio_text") != last_assistant_msg:
                with st.spinner("Falcon Voice Synthesis..."):
                    audio_data = generate_human_voice(last_assistant_msg)
                    if audio_data:
                        st.session_state["active_audio_bytes"] = audio_data
                        st.session_state["active_audio_text"] = last_assistant_msg

            # Render the Player
            if "active_audio_bytes" in st.session_state:
                st.audio(st.session_state["active_audio_bytes"], format="audio/mpeg", autoplay=True)
                st.caption("🔊 High-Fidelity Audio Feed Active")
   # protocol manual)
